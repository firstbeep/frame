...
| Working install and demo | A clean clone passed `npm install` and `npm test` on Windows x64. The worker handshake is checked separately by `npm run doctor`; real diffusion and upscale were checked by `npm run smoke` after model setup. A new reviewer machine still needs the documented native prerequisites and about 4 GB of model downloads. |
...
| X post links repo and tags `@qvac` | The linked [X post](https://x.com/firshbeepxvft/status/2103056780834308515) returned 403 to our read-only browser, so its text, repo link, tag, and attachments remain **unverified**. The X account handle differs from the GitHub handle, though the provided rules do not require them to match. |
